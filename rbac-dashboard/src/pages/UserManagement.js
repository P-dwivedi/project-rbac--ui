...
const UserManagement = () => {
    ...
    const handleInputChange = (e) => {
      ...
      const validateUser = (user) => {
        ...
        } else if (!/^[A-Za-z\s]+$/.test(user.role)) { 
      ...
      const handleAddUser = () => {
        ...
        if (Object.keys(validationErrors).length > 0) {
          setErrors(validationErrors); // To set errors if validation fails
          return; // To stop execution if validation fails
        }
    
        const id = users.length ? users[users.length - 1].id + 1 : 1;
        setUsers([...users, { ...newUser, id }]);
        setNewUser({ name: '', email: '', role: '', status: 'Active' }); // To reset form
        setErrors({}); // To clear errors after success
      };

      // To save the edited user
      const handleSaveEdit = () => {
        ...
        setEditingUser(null); // To exit editing mode
        setErrors({}); // To clear errors after success
      };

      // To cancel
      const handleCancelEdit = () => {
        setEditingUser(null); // To clear
        setErrors({}); // To clear errors when canceling
      };
      ...
      return (
        <div className="min-h-screen mx-10 bg-white text-gray-800">
