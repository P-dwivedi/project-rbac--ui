...
const RoleManagement = () => {
  const [roles, setRoles] = useState([]); // To store roles
  const [newRole, setNewRole] = useState({ name: '', permissions: [] }); // New role data
  const [editingRole, setEditingRole] = useState(null); // Role edited
  const [newPermission, setNewPermission] = useState(''); // Individual permission input

  // To handle input changes for role name
  ...
  // To handle adding a new permission
  ...
  // To handle removing a permission
  ...
  // To save changes to an edited role
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-800 rounded-xl">
      <div className="max-w-7xl mx-auto p-4">
        <h2 className="text-2xl font-bold mb-4 text-center">Role Management</h2>

        {/* Add/edit role section */}
        <div className="bg-white p-4 rounded shadow-xl mb-6">
            ...
            <tbody>
              {roles.map((role) => (
                <tr key={role.id} className="hover:bg-gray-100">
                  <td className="px-4 py-2 border border-gray-300">{role.id}</td>
